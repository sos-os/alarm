...
quickcheck! {
    fn not_empty_after_push_node(n: usize) -> bool {
        ...
        list.push_front_node(NumberedNode::new(n));

        !list.is_empty() && list.len() == 1

    }

    fn contents_after_first_push_node(n: usize) -> bool {
        let mut list = List::<usize, NumberedNode>::new();
        assert_eq!(list.head(), None);
        assert_eq!(list.tail(), None);
        list.push_front_node(NumberedNode::new(n));
    ...
    fn head_tail_same_first_push_node(n: usize) -> bool {
        let mut list = List::<usize, NumberedNode>::new();
    ...
    fn head_tail_not_same_second_push_node(a: usize, b: usize) -> TestResult {
        if a == b {
            return TestResult::discard()
        };
        let mut list = List::<usize, NumberedNode>::new();

        list.push_front_node(NumberedNode::new(a));
        list.push_front_node(NumberedNode::new(b));
    ...
    fn push_front_node_order(x: usize, xs: Vec<usize>) -> TestResult {
        let mut list = List::<usize, NumberedNode>::new();
        list.push_front_node(NumberedNode::new(x));
        let mut result = TestResult::passed();
        for x_2 in xs {
            list.push_front_node(NumberedNode::new(x_2));
...
fn contents_after_push_nodes() {
    ...
    list.push_front_node(NumberedNode::new(0));
    list.push_front_node(NumberedNode::new(1));

    assert_eq!(list.tail().unwrap().number, 0);
    assert_eq!(list.head().unwrap().number, 1);

    list.push_back_node(NumberedNode::new(2));
    assert_eq!(list.tail().unwrap().number, 2);
    assert_eq!(list.head().unwrap().number, 1);

    list.push_back_node(NumberedNode::new(3));
